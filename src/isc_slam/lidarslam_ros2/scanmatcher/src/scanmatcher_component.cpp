...
void ScanMatcherComponent::initializePubSub()
{
    ...
    [this](const typename sensor_msgs::msg::PointCloud2::SharedPtr msg) -> void
    {
      if (initial_pose_received_) {
        ...
        if (!initial_cloud_received_) {
          ...
          // create map topic and publish it
          sensor_msgs::msg::PointCloud2::SharedPtr map_msg_ptr(new sensor_msgs::msg::PointCloud2); // set map topic type
          pcl::toROSMsg(*transformed_cloud_ptr, *map_msg_ptr);

          // create map array and publish it
  ...
  // create initial_pose, imu, input_cloud subscribers 
  initial_pose_sub_ =
  ...
  input_cloud_sub_ =
   ...
   // create current_pose, map, map_array, path publishers
  ...
  const rclcpp::Time stamp)
{
  ...
  // create odom tranformation from odom to robot_frame_id
  if (use_odom_) {
  ...
  const Eigen::Matrix4f final_transformation, const rclcpp::Time stamp)
{
  ...
  transform_stamped.child_frame_id = odom_frame_id_;       // from map to odom tranformation
